        ...
        if (get_option('comment_registration') and ! is_user_logged_in()) {
            ...
            <form action="<?php echo get_option('siteurl') ?>/wp-comments-post.php" method="post" id="commentform">
                ...
                if (is_user_logged_in()) {
                    ...
                    ?>
                    <div>
                        <input type="text" name="author" id="author" value="<?php echo esc_attr($comment_author) ?>" size="22" tabindex="1"<?php echo ($req) ? " aria-required='true'" : ''; ?> />
                        <label for="author">Name<?php echo ($req) ? " (required)" : '' ?></label>
                    </div>

                    <div>
                        <input type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email) ?>" size="22" tabindex="2"<?php echo ($req) ? " aria-required='true'" : ''; ?> />
                        <label for="email">Mail (will not be published)<?php echo ($req) ? " (required)" : '' ?></label>
                    </div>
